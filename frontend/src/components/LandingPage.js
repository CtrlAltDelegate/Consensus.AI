...
function LandingPage({ onGetStarted, onLearnMore }) {
  return React.createElement('main', { className: 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20' },
    ...
    // Sample Output Preview — concrete excerpts of real consensus reports (highest-converting element)
    React.createElement('section', { id: 'sample-output', className: 'mb-20 scroll-mt-8' },
      React.createElement('div', { className: 'text-center mb-12' },
        React.createElement('h2', { className: 'text-4xl font-bold text-slate-900 mb-4' }, 'See What a Consensus Report Looks Like'),
        React.createElement('p', { className: 'text-xl text-slate-600 max-w-2xl mx-auto' },
          'Real excerpts from reports generated by Consensus.AI — balanced analysis from GPT-4o, Claude, Gemini, and Cohere.'
        )
      ),
      React.createElement('div', { className: 'grid md:grid-cols-1 lg:grid-cols-3 gap-8' },

        // Sample 1: Competitive analysis
        React.createElement('div', { className: 'bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col' },
          React.createElement('div', { className: 'px-5 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between' },
            React.createElement('h3', { className: 'font-semibold text-slate-900' }, 'Competitive analysis'),
            React.createElement('span', { className: 'text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded' }, '4-LLM consensus')
          ),
          React.createElement('div', { className: 'p-5 flex-1 flex flex-col' },
            React.createElement('p', { className: 'text-sm text-slate-500 mb-3' }, 'Product A vs Product B — enterprise buyers'),
            React.createElement('div', { className: 'text-slate-700 text-sm leading-relaxed space-y-3 flex-1' },
              React.createElement('p', null,
                'Across all models, the consensus is that Product A leads on security and compliance (SSO, audit logs, SOC 2), while Product B offers stronger integration breadth and lower entry cost. Enterprise buyers prioritizing governance should favor A; those prioritizing time-to-value and ecosystem may prefer B.'
              ),
              React.createElement('p', null,
                'All four perspectives agreed that neither product dominates on support quality; G2 and internal feedback suggest support is a swing factor. Recommendation: run a 30-day pilot with both if the decision is material, with clear criteria on integration and compliance.'
              )
            )
          )
        ),

        // Sample 2: Strategic decision pros/cons
        React.createElement('div', { className: 'bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col' },
          React.createElement('div', { className: 'px-5 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between' },
            React.createElement('h3', { className: 'font-semibold text-slate-900' }, 'Strategic decision'),
            React.createElement('span', { className: 'text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded' }, '4-LLM consensus')
          ),
          React.createElement('div', { className: 'p-5 flex-1 flex flex-col' },
            React.createElement('p', { className: 'text-sm text-slate-500 mb-3' }, 'Build vs buy for core platform'),
            React.createElement('div', { className: 'text-slate-700 text-sm leading-relaxed space-y-3 flex-1' },
              React.createElement('p', null,
                'The models converged on a nuanced view: build when differentiation and long-term control matter and you have in-house capacity; buy when speed and proven reliability outweigh customization. All four highlighted vendor lock-in and TCO over 3–5 years as the main risks of buy.'
              ),
              React.createElement('p', null,
                'Consensus recommendation: pursue a hybrid — buy for commodity capabilities (auth, billing) and build for the 2–3 workflows that define your product. Confidence was highest where internal data on team size and roadmap was provided.'
              )
            )
          )
        ),

        // Sample 3: Market / trend synthesis
        React.createElement('div', { className: 'bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col' },
          React.createElement('div', { className: 'px-5 py-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between' },
            React.createElement('h3', { className: 'font-semibold text-slate-900' }, 'Market synthesis'),
            React.createElement('span', { className: 'text-xs font-medium text-indigo-600 bg-indigo-50 px-2 py-1 rounded' }, '4-LLM consensus')
          ),
          React.createElement('div', { className: 'p-5 flex-1 flex flex-col' },
            React.createElement('p', { className: 'text-sm text-slate-500 mb-3' }, 'Role of nuclear in net-zero pathways'),
            React.createElement('div', { className: 'text-slate-700 text-sm leading-relaxed space-y-3 flex-1' },
              React.createElement('p', null,
                'Synthesis of IPCC, IEA, and regional studies produced strong agreement: nuclear can contribute to low-carbon baseload, but cost and build timelines vary sharply by region. Divergence appeared on SMRs — some models emphasized near-term potential; others stressed regulatory and supply-chain uncertainty.'
              ),
              React.createElement('p', null,
                'Consensus: include nuclear in scenario planning for net-zero; do not assume it fills the gap without policy and supply-chain progress. Best used as one of several levers alongside renewables, storage, and demand reduction.'
              )
            )
          )
        )
      ...
      React.createElement('div', { className: 'mt-8 text-center' },
        React.createElement('p', { className: 'text-sm text-slate-500' },
          'Full reports include confidence scores, methodology, and PDF export. These are sanitized excerpts.'
        ),
        React.createElement('button', {
          onClick: onGetStarted,
